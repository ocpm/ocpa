...
event_df = pd.read_csv(filename,sep=',')[:1000]
print(event_df)
for ot in ots:
...
sub_ocel = ocel#trace_filtering.filter_infrequent_traces(ocel,0.3)
...
for i in range(0,25):
    precision, fitness = precision_fitness_evaluator.apply(ocel,ocpn,contexts=contexts,bindings=bindings)
