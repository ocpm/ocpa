...
if False:
    ...
    data_df = pd.DataFrame(
        {"date": feat_to_s[list(feat_to_s.keys())[0]][0][1]})
    for feat in feat_to_s.keys():
        ...
        data_df[name] = [feat_to_s[feat][0][0][i]
                         for i in range(0, len(feat_to_s[feat][0][0]))]
    plt.clf()
    ...
    plot_ = sns.lineplot(
        data=data_df["Average Number of Objects"], color="#4C72B0", linewidth=2)
    ax2 = plt.twinx()
    sns.lineplot(data=data_df["Average Remaining Time in s"],
                 ax=ax2, color="#DD8452", linewidth=2)
    ...
    plot_.set_ylabel("Average Number of Offer Objects",
                     color="#4C72B0", fontsize=11)
...
if True:
    ...
    if True:
        explainer = shap.Explainer(
            model.predict, x_train, feature_names=x_train.columns.tolist())
...
if False:
    ...
    #activities = list(set(ocel.log["event_activity"].tolist()))
    F3 = [(feature_extraction.EVENT_ACTIVITY, (act,)) for act in activities] + \
        [(feature_extraction.EVENT_TYPE_COUNT, (ot,)) for ot in ots]
...
if True:
    ...
    ##feature_storage = feature_extraction.apply(ocel, F, [])
    ##print("set Features")
    features = [feat for feat in F if feat != (
        feature_extraction.EVENT_REMAINING_TIME, ())]
    target = (feature_extraction.EVENT_REMAINING_TIME, ())
    train_sequences = sequential.construct_sequence(
        feature_storage, index_list=feature_storage.training_indices)
    test_sequences = sequential.construct_sequence(
        feature_storage, index_list=feature_storage.test_indices)

    x_train, y_train = sequential.construct_k_dataset(
        train_sequences, k, features, target)
    x_test, y_test = sequential.construct_k_dataset(
        test_sequences, k, features, target)
    ...
    explainer = shap.KernelExplainer(f, X100.reshape(
        X100.shape[0], X100.shape[1]*X100.shape[2]))
    shap_values = explainer.shap_values(x_train[:number_shap].reshape(
        x_train[:number_shap].shape[0], x_train[3].shape[0]*x_train[3].shape[1]), nsamples=100)
    shap_values = shap_values[0].reshape(
        shap_values[0].shape[0], k, int(shap_values[0].shape[1]/k))
    print(shap_values[0])
    mapping_names = {feature_extraction.EVENT_PRECEDING_ACTIVITES: "Prec. activities:",
                     feature_extraction.EVENT_ELAPSED_TIME: "Elapsed time",
                     feature_extraction.EVENT_PREVIOUS_TYPE_COUNT: "Previous objects of:",
    ...
    for i in range(0, len(shap_values)):
        ...
        for i_c in range(0, len(curr_order)):
            ...
            for i_f in range(0, k):
                new_shap[i_f][mapping_from_curr_to_new[feat]
                              ] = shap_values[i][i_f][i_c]

        ax = sns.heatmap(np.transpose(new_shap), xticklabels=[
                         "-3 events", "-2 events", "-1 event", "current event"], yticklabels=label_order)
...
if True:
    ...
    x_train, y_train = generate_graph_dataset(
        feature_storage.feature_graphs, feature_storage.training_indices, ocel)
    ...
    for e in range(epochs):
        ...
        for batch_id in tqdm(iter_idx):
            ...
            if (step % 100 == 0):
                print('Loss: %s' % ((current_loss / step)))
    ...
    print(tf.keras.metrics.mean_absolute_error(np.squeeze(
        test_labels), np.squeeze(test_predictions)).numpy())
    ...
    def f(edge_selection):
        ...
        for i in edge_selection:
            idx = np.concatenate(
                [i, np.array([1, 1, 1, 1])], axis=0).astype('bool')
            ...
            with tf.device('CPU:0'):
                pred = model(
                    new_graph, new_graph.ndata['features']).numpy().squeeze()
