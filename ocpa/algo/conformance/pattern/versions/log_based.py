...
def apply(pg: PatternGraph, ocel: ObjectCentricEventLog, eocpn: EnhancedObjectCentricPetriNet, parameters=None) -> List[str]:
    evals = []
    for cf_edge in pg.cf_edges:
        evals.append(evaluate_cf_edge(ocel, cf_edge))

    for obj_edge in pg.obj_edges:
        evals.append(evaluate_or_edge(ocel, obj_edge))
