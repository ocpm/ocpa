...
class Feature_Storage:
    class Feature_Graph:
        ...
        def __init__(self, case_id, graph, ocel):
            ...
            self._nodes = [Feature_Storage.Feature_Graph.Node(e_id) for e_id in graph.nodes]
            objects = {(source,target):set(ocel.log.loc[source]["event_objects"]).intersection(set(ocel.log.loc[target]["event_objects"])) for source,target in graph.edges}
            self._edges = [Feature_Storage.Feature_Graph.Edge(source,target, objects = objects[(source,target)])for source,target in graph.edges]
            self._attributes = {}
        def _get_nodes(self):
            return self._nodes
